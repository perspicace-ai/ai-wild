...
import data_management.tfrecords.utils.create_tfrecords_v2 as tfr
#else:
...
##unclear why it is important to assert the presence of data we've said to ignore args.exclude_categories
...
class TFRecordsWriter(object):
  ...
  def add(self, data):
    if self.next_shard_img_idx % self.ims_per_record == 0:
      if self.writer:
        self.writer.close()
      self.writer = tf.io.TFRecordWriter(self.output_file%self.next_shard_idx)
